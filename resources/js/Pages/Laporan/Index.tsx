...
export default function ReportIndex({ auth, date, reportData }: PageProps<{ date: string, reportData: ReportData }>) {
   ...
   const saldoAkhirMandiri = (saldo_awal.mandiri || 0)
    ...
    const totalSaldoAkhir = saldoAkhirKas + saldoAkhirBca + saldoAkhirMandiri + (ops?.ghost_adjustment || 0);
                    ...
                    <CardContent className="pt-6 space-y-6">
                        ...
                        <div className="bg-gray-50 dark:bg-zinc-800/50 rounded-lg p-4 space-y-3 border border-gray-100 dark:border-zinc-800">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className={`font-bold ${((mutations.salesBca || 0) - (mutations.buyBca || 0) + (ops?.bca_in || 0) - (ops?.bca_out || 0)) < 0 ? 'text-red-600' : 'text-white/80'}`}>
                            ...
                            <div className="flex justify-between text-sm">
                                ...
                                <span className={`font-bold ${((mutations.salesMandiri || 0) - (mutations.buyMandiri || 0) + (ops?.mandiri_in || 0) - (ops?.mandiri_out || 0)) < 0 ? 'text-red-600' : 'text-white/80'}`}>
